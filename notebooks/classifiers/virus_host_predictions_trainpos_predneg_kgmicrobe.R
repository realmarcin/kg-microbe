...
node_data <- read.csv("./20210119/merged-kg/merged-kg_nodes.tsv", sep="\t",header=T)
...
if(!done) {
  ...
  capture.output(rr, file="kgmicrobe_sample.txt")
  save(rr, file="kgmicrobeR_sample.RData")
  ...
  sample <- load("kgmicrobe_sample.RData")
...
if(!done) {
  write.table(row.names(test), file="kgmicrobe_test_edges_labels.txt", sep="\t")
}
